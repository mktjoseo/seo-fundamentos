...
exports.handler = async function(event, context) {
  ...
  try {
    ...
    if (!USER_SERPER_API_KEY || !USER_GEMINI_API_KEY) {
      throw new Error('Las claves de Serper y Gemini deben estar configuradas en el servidor.');
    }
    
    const { keyword, articleText, projectId } = JSON.parse(event.body);
    if (!keyword || !articleText) {
      throw new Error('La palabra clave y el texto del artículo son requeridos.');
    }
    if (!projectId) {
    ...
    if (!serperResponse.ok) {
      throw new Error('Error al llamar a la API de Serper.');
    }
